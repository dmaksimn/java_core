...
public class FabricOpel extends CarFabric {
    ...
    public FabricOpel(ServiceChangeColor serviceColor, ServiceChangeWheel serviceChangeWheel
        ...
        carStorageOpel.add(new CarOpel(CarColor.GREEN, CarBody.Sedan, CarModel.Corsa, 2015, CarSizeWheels.Low, CarEngineVolume.Big));
        carStorageOpel.add(new CarOpel(CarColor.RED, CarBody.Limousine, CarModel.Zafira, 2016, CarSizeWheels.BigLarge, CarEngineVolume.RaceCar));
    }
    ...
    public Car createCar(CarColor color, CarBody body, CarModel model, int year, CarSizeWheels sizeWheels, CarEngineVolume engineVolume) throws IllegalArgumentException {
        if (!sizeWheels.toString("Low", "Medium", "Large", "BigLarge", "Monster", "BigFoot").equals(sizeWheels.getSizeWheels())) {
            ...
            for (int i = 0; i < carStorageOpel.size(); i++) {
                if (carStorageOpel.get(i).compareCarInStorage(color, body, model, year, sizeWheels, engineVolume)) {
