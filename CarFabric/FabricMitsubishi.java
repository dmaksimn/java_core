...
public class FabricMitsubishi extends CarFabric {
    ...
    public FabricMitsubishi(ServiceChangeColor serviceColor, ServiceChangeWheel serviceChangeWheel
        ...
        carStorageMitsubishi.add(new CarMitsubishi(CarColor.BLUE, CarBody.SUV, CarModel.Lancer, 2013, CarSizeWheels.Medium, CarEngineVolume.Small));
        carStorageMitsubishi.add(new CarMitsubishi(CarColor.WHITE, CarBody.Pickup, CarModel.ASX, 2014, CarSizeWheels.Large, CarEngineVolume.TrackCar));
    }
    ...
    public Car createCar (CarColor color, CarBody body, CarModel model, int year, CarSizeWheels sizeWheels, CarEngineVolume engineVolume)  throws IllegalArgumentException {
        if (!sizeWheels.toString("Low", "Medium", "Large", "BigLarge", "Monster", "BigFoot").equals(sizeWheels.getSizeWheels())) {
            ...
            for (int i = 0; i < carStorageMitsubishi.size(); i++) {
                if (carStorageMitsubishi.get(i).compareCarInStorage(color, body, model, year, sizeWheels, engineVolume)) {
